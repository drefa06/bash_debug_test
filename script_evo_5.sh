...
gen_good_1()
{
    echo "gen_good_1: START" >> ${LOGFILE} # log only

    sleep 2

    value1="bar"
    value2="bar"
    echo "test gen_good_1: if [ $value1 == $value2 ]"
    if [ $value1 == $value2 ]; then
        echo "$value1==$value2"
    else
        echo "$value1!=$value2"
    fi
    echo "gen_good_1: No Error, continue script" | tee -a ${LOGFILE} #log and screen
    echo "gen_good_1: END" >> ${LOGFILE}
